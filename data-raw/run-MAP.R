...
ref.exposure <- PCAWG7::exposure$PCAWG$SBS96[ , colnames(spect), drop = FALSE]

foo <- GetExposureWithConfidence(catalog = spect,
                                 sig.universe = PCAWG7::signature$genome$SBS96[ , names(sigs.prop)],
...
mapout <-
  mSigAct::MAPAssignActivity1(
    spect = spect,
    sigs = PCAWG7::signature$genome$SBS96[ , rownames(foo)],
    sigs.presence.prop = sigs.prop[rownames(foo)],
...
QP.exp <- mSigAct:::OptimizeExposureQP(spect, PCAWG7::signature$genome$SBS96[ , names(bx)])
...
r.qp <- ReconstructSpectrum(PCAWG7::signature$genome$SBS96, exp = QP.exp, use.sig.names = TRUE)
...
sol.matrix <- cbind(spect, r.p, r.b, r.qp)
colnames(sol.matrix) <- c("spect", "PCAWG7", "MAP.best",  "MAP.best.QP.obj.fn")
