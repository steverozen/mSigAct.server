# Source this file from mSigAct.server package root

cat(getwd(), "\n")

tmp.SBS96 <- 
  data.table::fread("data-raw/package-data-related/data/COSMIC-v3-SBS-proposed-aetiology-short-form.csv")
...
sigs.etiologies.SBS192 <- 
  matrix(tmp.SBS192$proposed.aetiology, ncol = 1, 
         dimnames = list(mSigAct:::AddMinusEFor192(tmp.SBS96$name), "proposed.etiology"))

tmp.DBS78 <- 
  data.table::fread("data-raw/package-data-related/data/COSMIC-v3-DBS-proposed-aetiology-short-form.csv")
sigs.etiologies.DBS78 <- 
  matrix(tmp.DBS78$proposed.aetiology, ncol = 1, 
         dimnames = list(tmp.DBS78$name, "proposed.etiology"))

tmp.ID <- 
  data.table::fread("data-raw/package-data-related/data/COSMIC-v3-ID-proposed-aetiology-short-form.csv")
sigs.etiologies.ID <- 
